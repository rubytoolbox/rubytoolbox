  ...
  describe "with posts from fixtures" do
    ...
    describe "#posts" do
      ...
      it "is sorted by published_on DESC" do
        expect(blog.posts).to eq blog.posts.sort_by(&:published_on).reverse
      end

      it "has expected post published_on dates" do
        expect(blog.posts.map(&:published_on)).to eq [Date.new(2018, 1, 12), Date.new(2017, 12, 1)]
      end

      it "has expected post permalinks" do
        expect(blog.posts.map(&:permalink)).to eq %w[another-post example-post]
      end

      it "has expected post titles" do
        expect(blog.posts.map(&:title)).to eq ["Another title", "Example Title"]
      end
      ...
      it "has expected post html bodies" do
        ...
        expect(blog.posts.map(&:body_html)).to eq expected_bodies
      end
    end
    ...
    describe "#recent_posts" do
      it "contains posts published less than two months ago" do
        ...
        expect(blog.recent_posts).to eq [recent_post]
      end
      ...
      it "returns at most 3 recent posts" do
        ...
        expect(blog.recent_posts).to eq [post, post, post]
      end
    end

    describe ".find" do
      it "can find a given post by :published_on/:permalink" do
        expect(blog.find("2018-01-12/another-post")).to eq blog.posts.first
      end
